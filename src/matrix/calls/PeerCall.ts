...
interface PeerCallHandler {
    emitUpdate();
    sendSignallingMessage(type: string, content: Record<string, any>);
}
...
class PeerCall implements PeerConnectionHandler {
    ...
    onNegotiationNeeded() {
        ...
        this.handler.sendSignallingMessage(CallSetupMessageType.Invite, message);
    }
...
export class MatrixCall extends EventEmitter {
    // should go into DirectCall/GroupCall class
    ...
    // will happen if the opponent sends candidates eagerly before the user answers
    // the call) we buffer them up here so we can then add the ones from the party we pick
    private remoteCandidateBuffer: RTCIceCandidate[] = [];
