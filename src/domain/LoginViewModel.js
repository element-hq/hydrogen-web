...
export class LoginViewModel extends ViewModel {
    ...
    getSSOLink(homeserver) {
        const link = `${homeserver}/_matrix/client/r0/login/sso/redirect?redirectUrl=${this.urlCreator.createSSOCallbackURL()}`;
        return link;
    }

    async startSSOLogin(homeserver) {
        const hs = normalizeHomeserver(homeserver);
        await this.platform.settingsStorage.setString("homeserver", hs);
        this.platform.openUrl(this.getSSOLink(hs));
    }
